...
namespace {
    bool operator==(const parse::Token& token, char parsed_char_) {
        const auto* ptr_tkn = token.TryAs<TokenType::Char>();
        return ptr_tkn != nullptr && ptr_tkn->value == parsed_char_;
    }

    bool operator!=(const parse::Token& token, char parsed_char_) {
        return !(token == parsed_char_);
    }
